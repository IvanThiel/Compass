...
class CompassView extends Ui.DataField {
  ...
  function getSettings() {
    try {
      ...
      if (app != null) {
        var property = app.getProperty("windspeed");
        if (property != null && property instanceof Number) {
          mWindSpeedUnit = property;
        } else {
          mWindSpeedUnit = 0; // Default waarde
        }
      }
    } catch (ex) {
      debug("getSettings error: " + ex.getErrorMessage());
      mWindSpeedUnit = 0; // Fallback naar default
    }
  }
  ...
  function createRGB(r, g, b) {
    try {
      ...
      debug("createRGB error: " + ex.getErrorMessage());
  ...
  function getRGB(hex) {
    try {
      ...
      debug("getRGB error: " + ex.getErrorMessage());
      return [255, 255, 255];
    }
  }
  ...
  function fillRectangleGrad(dc, x, y, w, h, mh, rs, gs, bs, re, ge, be) {
    try {
      ...
      for (var i = 0; i < h && i < mh; i++) {
        try {
          var color = createRGB(rs + i * dr, gs + i * dg, bs + i * db);
          dc.setColor(color, Gfx.COLOR_TRANSPARENT);
          dc.drawLine(x, y - i, x + w, y - i);
        } catch (ex) {
          debug("fillRectangleGrad inner error: " + ex.getErrorMessage());
          break;
        }
      }
    } catch (ex) {
      debug("fillRectangleGrad error: " + ex.getErrorMessage());
    }
  }
  /******************************************************************
   * LABELS
  ...
  function drawSmallArrow(dc, track, bearing, xx, yy) {
    try {
      ...
      if (bearing != null && track != null) {
        ...
        var x1_3 = Math.round(x + r * Math.cos((angle + 90) * _PI_180) * l1);
        var y1_3 = Math.round(y + r * Math.sin((angle + 90) * _PI_180) * l1);

        var x2_3 = Math.round(r * Math.cos(angle * _PI_180) * l2);
        var y2_3 = Math.round(r * Math.sin(angle * _PI_180) * l2);

        var x3_3 = Math.round(x + r * Math.cos((angle - 90) * _PI_180) * l1);
        var y3_3 = Math.round(y + r * Math.sin((angle - 90) * _PI_180) * l1);

        var x4_3 = Math.round(r * Math.cos(angle * _PI_180) * -l2);
        var y4_3 = Math.round(r * Math.sin(angle * _PI_180) * -l2);

        var x5_3 = Math.round(r * Math.cos(angle * _PI_180) * 0.15);
        var y5_3 = Math.round(r * Math.sin(angle * _PI_180) * 0.15);

        dc.setPenWidth(2);
        dc.setColor(Graphics.COLOR_LT_GRAY, Graphics.COLOR_TRANSPARENT);
        dc.drawCircle(xx, yy, r);

        if (bearing != null) {
          dc.setColor(Graphics.COLOR_BLACK, Graphics.COLOR_TRANSPARENT);
          dc.setPenWidth(3);
          dc.drawLine(x5_3 + xx, y5_3 + yy, x4_3 + xx, y4_3 + yy);
      ...
      debug("drawSmallArrow error: " + ex.getErrorMessage());
    }
  }
  ...
  function drawForecast(dc) {
    try {
      ...
      try {
        c = Weather.getHourlyForecast();
      } catch (ex) {
        debug("Weather forecast not available: " + ex.getErrorMessage());
        return false;
      }
      if (c == null) {
        // Toon een bericht dat weather data niet beschikbaar is
        setStdColor(dc);
        dc.drawText(
          mW / 2,
          mH / 2,
      ...
      for (var t = 0; t < 3; t++) {
        ...
        for (var i = 0; i < n; i++) {
          ...
          RGB_S = [
            RGB_S[0] + lighter * (255 - RGB_S[0]),
            RGB_S[1] + lighter * (255 - RGB_S[1]),
            RGB_S[2] + lighter * (255 - RGB_S[2]),
          ];
      ...
      */
      ...
      try {
        ...
        if (mDistance != null) {
          ...
          var dist = mDistance;
          if (dist >= 1000) {
            v2 = Math.round(dist / 1000).format("%i") + "km";
          }
          if (dist < 1000) {
            v2 = Math.round(dist).format("%i") + "m";
          }
          dc.drawText(
            mW - XMARGING,
            mH - h[1],
  ...
  function drawWeather(dc) {
    try {
      if (!mShowCompass) {
        var success = drawForecast(dc);
        if (!success) {
          // Fallback naar compass view als forecast faalt
          drawWind(dc);
        }
      } else {
        drawWind(dc);
      ...
      debug("drawWeather error: " + ex.getErrorMessage());
    }
  }
  ...
  function compute(info) {
    try {
      ...
      // Veilige null checks voor alle info properties
      mAmbientPressure = 0;
      ...
      // Distance berekening met extra validatie
      mHome = null;
      mDistance = null;
      try {
        if (
          ...
          info has :startLocation &&
          info.startLocation != null
        ) {
          if (info.currentLocationAccuracy >= 2) {
            var startLoc = info.startLocation.toRadians();
            var currentLoc = info.currentLocation.toRadians();

            if (
              startLoc.size() >= 2 &&
              currentLoc.size() >= 2
            ) {
              var latitude_point_arrive = startLoc[0];
              var longitude_point_arrive = startLoc[1];
              var latitude_point_start = currentLoc[0];
              var longitude_point_start = currentLoc[1];

              // Valideer dat de coordinaten geldig zijn
              if (
                latitude_point_arrive != null &&
                longitude_point_arrive != null &&
                latitude_point_start != null &&
                longitude_point_start != null
              ) {
                var cosDistance =
                  Math.sin(latitude_point_start) *
                ...
                // Clamp de waarde om acos errors te voorkomen
                if (cosDistance > 1.0) {
                  cosDistance = 1.0;
                }
                if (cosDistance < -1.0) {
                ...
                mDistance = Math.acos(cosDistance);

                if (RAY_EARTH * mDistance > 0) {
                  var cosHome =
                    (Math.sin(latitude_point_arrive) -
                      Math.sin(latitude_point_start) * Math.cos(mDistance)) /
                    (Math.sin(mDistance) * Math.cos(latitude_point_start));
                  ...
                  mHome = Math.acos(cosHome);

                  if (
                    Math.sin(longitude_point_arrive - longitude_point_start) <=
                    0
                  ) {
        ...
        debug("Distance calculation error: " + ex.getErrorMessage());
        mHome = null;
        mDistance = null;
      }
      setWeather();
  ...
  function onUpdate(dc) {
    try {
      ...
      try {
        ...
        debug("Clear screen error: " + ex.getErrorMessage());
      }
      setStdColor(dc);
      drawWeather(dc);
    } catch (ex) {
      debug("onUpdate error: " + ex.getErrorMessage());

      // Emergency fallback - teken minimale info
      try {
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_BLACK);
        dc.clear();
        dc.drawText(
        ...
        // Als zelfs de fallback faalt, doe niets
      }
    }
  }
}
...
function debug(message) {
  if (_debug) {
    try {
      ...
      // Zelfs debug kan falen, negeer dan
    }
  }
}
